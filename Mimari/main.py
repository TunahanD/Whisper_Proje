...
CONFIG = {
    ...
    "audioFile": "LightMode.wav",  # WAV dosyasının yolu
    
    "modelSize": "medium",     # small / medium / large-v2 / large-v3
    "device": "cpu",          # "cuda" ya da "cpu"
    "computeType": "float32",  # GPU için "float16", CPU için "int8"/"float32"
...
# Transkripsiyon
# =============================
def transcribe_wav(file_input,return_text=False):
    # WAV oku
    audio_np, sr = sf.read(file_input, dtype="float32")
    if audio_np.ndim > 1:
    ...
    print(f"[STT] ⚡ Çözümleme başlıyor: {file_input}")
